...
export default function AddJobAdvertisement() {
    ...
    function alertUser(data){
        if(data.successful){
            alertify.success("Başarılı bir şekilde eklendi. HRMS personelimiz onayladıktan sonra ilanınız yayında olacaktır. Teşekkür ederiz.")
        }
        else{
            alertify.error("Beklenmedik bir hata oluştu. Lütfen daha sonra tekrar deneyin")
        }
    }
            ...
            <div className="form-container">
                ...
                <Formik
                    ...
                    validationSchema={Yup.object({
                        ...
                        openPositionNumber: Yup.number().min(1,"Açık pozisyon sayısı en az 1 olmalıdır"),//.required('Açık pozisyon sayısını girmelisiniz'),
                        deadline: Yup.date().required('Son geçerlilik tarihini girmelisiniz').min(new Date(Date.now()),"Geçersiz tarih"),
                        employer: Yup.object({
                            id:Yup.number()
                        }),
                        jobTitle: Yup.object({
                        ...
                        description:Yup.string().min(10,"En az 10 karakter girmelisiniz").required("Açıklama boş olamaz")
                    })}
                    onSubmit={(values,{resetForm}) => {
                        //console.log(values)
                        let jobAdvertisementService=new JobAdvertisementService()
                        jobAdvertisementService.postJobAdvertisement(values).then(response=>(alertUser(response.data)))
                        resetForm()
                            ...
                            <div>
                                <div className="city-job-way-type-container">
                                        ...
                                        <div className="select-container input-select">
                                            ...
                                            >
                                                ...
                                                {jobTitles.map((jobTitle)=>(
                                                    <option value={jobTitle.id} key={jobTitle.id} label={jobTitle.title}/>
                                                ))}
                                            </select>
                                        </div>
                                        ...
                                        <div className="select-container input-select">
                                            ...
                                            >
                                                ...
                                                {wayOfWorks.map((wayOfWork)=>(
                                                    <option value={wayOfWork.id} key={wayOfWork.id} label={wayOfWork.wayOfWork}/>
                                                ))}
                                            </select>
                                            {errors.wayOfWork?.id && touched.wayOfWork?.id && (
                                                <div className="input-feedback">{errors.wayOfWork.id}</div>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="type-of-work-form form-item">
                                        ...
                                        <div className="select-container input-select" id="type-of-work-select">
                                            <select
                                                id="typeOfWork.id"
                                            ...
                                            >
                                                ...
                                                {typeOfWorks.map((typeOfWork)=>(
                                                    <option value={typeOfWork.id} key={typeOfWork.id} label={typeOfWork.workType}/>
                                                ))}
                                            </select>
                                        </div>
                                        {errors.typeOfWork?.id && touched.typeOfWork?.id && (
                                            <div className="input-feedback">{errors.typeOfWork.id}</div>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="open-position-deadline-container">
                                    <div className="open-position-number-form form-item">
                                        ...
                                        {errors.openPositionNumber && touched.openPositionNumber && (
                                            <div className="input-feedback">{errors.openPositionNumber}</div>
                                        )}
                                    </div>
                                ...
                                <div className="description-form form-item">
                                    ...
                                    <div className="text-area-container input-select">
                                        <textarea name="description" id="description"  placeholder="Açıklama" value={values.description} onChange={handleChange}></textarea>
                                    </div>
